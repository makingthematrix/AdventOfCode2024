...
object DaySix:
  @tailrec
  private def getPath(x: Int, y: Int, dir: (Int, Int) = (-1, 0), lst: List[(Int, Int)] = Nil)
                     (using arr: Array[Char], len: Int): List[(Int, Int)] =
  ...
  @main def main(): Unit =
    val (array, len) = readLines("input6").pipe { lines => (lines.mkString.toCharArray, lines.head.length) }
