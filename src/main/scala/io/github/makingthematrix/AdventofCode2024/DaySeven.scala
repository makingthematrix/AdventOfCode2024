...
final case class Line(testValue: Long, numbers: Array[Long]):
  private def applyOps(ops: List[Op]): Long = numbers.tail.zip(ops).foldLeft(numbers.head) { case (a, (b, op)) => op(a, b) }
  def isCalibrated(ops: LazyList[Vector[List[Op]]]): Boolean = ops(numbers.length - 1).exists(applyOps(_) == testValue)

object DaySeven:
  private def lazyListLoop(ops: List[Op], n: Int = 0): LazyList[Vector[List[Op]]] =
    (0 until n).foldLeft(Vector(List.empty[Op])) { (lst, _) => lst.flatMap(l => ops.map(_ :: l)) } #:: lazyListLoop(ops, n + 1)

  private def sumCalibrated(lines: Seq[Line], ops: List[Op]): Long =
    val allOps = lazyListLoop(ops)
    lines.par.collect { case line if line.isCalibrated(allOps) => line.testValue }.sum

  @main def main(): Unit =
    val lines = readLines("input7").collect { case s"$value: $numbers" => Line(value.toLong, numbers.split(' ').map(_.toLong)) }
    // Part 1
